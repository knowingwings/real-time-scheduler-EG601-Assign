...
Implements a preemptive priority-based scheduling algorithm without
priority inversion handling, for comparison purposes.
"""
...
class PriorityBasicScheduler:
    """
    Basic Priority-Based Scheduler
    
    Tasks are executed based on their priority, with higher priority tasks
    preempting lower priority ones. No priority inversion handling is implemented.
    """
    ...
    def __init__(self):
        ...
        self.metrics = {
            ...
            'memory_usage': [],
            'timestamp': [],
            'priority_inversions_detected': 0,  # We still detect but don't handle them
        }
    ...
    def add_task(self, task):
        ...
        self.logger.info(f"Task {task.id} added to Basic Priority queue with priority {task.priority.name}")
    ...
    def allocate_resource(self, task, resource_id):
        ...
        but does not implement any handling mechanism.
        """
        with self.lock:
            if resource_id in self.resources and self.resources[resource_id]['owner']:
                ...
                # Check for potential priority inversion (detect but don't handle)
                if task.priority.value < owner.priority.value:  # Lower value = higher priority
                    ...
                    self.metrics['priority_inversions_detected'] += 1
                    
                    # No priority inheritance is applied here
    ...
    def release_resource(self, task, resource_id):
        ...
        Simple release without priority inheritance
        """
        with self.lock:
            if (resource_id in self.resources and 
                ...
                if waiters:
                    # Allocate to next waiter (no priority consideration)
    ...
    def get_metrics(self):
        ...
        with self.lock:
            ...
            return {
                ...
                'priority_inversions_detected': self.metrics['priority_inversions_detected'],
                'priority_inversions_handled': 0  # Always 0 since we don't handle them
            }